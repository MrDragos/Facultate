...
(define dummy-agent
  (λ (goal world-state rest-actions memory)
    ;(display (_value-of 'room world-state))
   ; (display (get-agent-color world-state))
    ;(display "\n")
    `(,(car (foldl (λ (i acc)
...
(define (permutations size elem)
  (cond
    [(= 0 size) '(())]
    [else (append-map (λ (x) (map (lambda (y) (cons y x)) elem)) (permutations (- size 1) elem))]
          ))
...
(define test-state '( 
                     ...
                     (door room1 blueWarehouse) 
                     (door room1 room2) 
                     (door room2 room1)
                     (door room2 redWarehouse) 
                     (door redWarehouse room2) 
                     ;;spheres
                     (spheres gray blueWarehouse 1)
                     (spheres red blueWarehouse 0) 
                     (spheres blue blueWarehouse 0)
                     (spheres gray room1 1) 
                     (spheres red room1 2) 
                     (spheres blue room1 2) 
                     (spheres gray room2 1) 
                     (spheres red room2 2) 
                     (spheres blue room2 2) 
                     (spheres gray redWarehouse 1)
                     (spheres red redWarehouse 0) 
                     ...
                     (carries gray 0)
                     ;;location
                     (location redWarehouse)
                     ;;robot color
                     (color red)))

(define final-state '( (color blueWarehouse blue) (color redWarehouse red) (color room1 white) (color room2 white) (door blueWarehouse room1) (door room1 blueWarehouse) (door room1 room2) (door room2 room1) (door room2 redWarehouse) (door redWarehouse room2) (spheres gray blueWarehouse 1) (spheres red blueWarehouse 0) (spheres blue blueWarehouse 0) (spheres gray room1 1) (spheres red room1 2) (spheres blue room1 2) (spheres gray room2 1) (spheres red room2 2) (spheres blue room2 2) (spheres gray redWarehouse 1) (spheres red redWarehouse 0) (spheres blue redWarehouse 0) (carries red 0) (carries blue 0) (carries gray 0) (location redWarehouse) (color red)))

;(filter (λ (x) (equal? (first x) 'carries)) test-state)
...
(define update-agent-state
  (λ (color world-state)
    (append (filter (λ (x) (equal? (first x) 'carries)) world-state)
...
(define finished?
  (λ (color goal world-state)
    ;(let* ([goal-warehouse (_warehouse color world-state)])
      (= (fourth goal) (_in-room-spheres-color (second goal) (third goal) world-state))
        )
    )
 ; )

(define get-agent-color
  (λ (world-state)
    (cadar (filter (λ (x) (and (eq? (car x) 'color) (= (length x) 2))) world-state))
    ))

(display "You know what it is !")
(get-agent-color test-state)
...
;;criteriul de alegere al actiunilor din A
;;pentru inceput va fi primul element, apoi vom adauga o euristica
(define choose-action 
  (λ (A)
    (car A)
    ))

;; A = lista de actiuni posibile cu preconditiile satisfacute
;; Path = lista actiunilor executate
;; _initial-agent-state

(define forward
  (λ (world-state goal A Path agent-state agent-color)
    (let* ([Act (choose-action A)]
           [new-world-state (car (_apply-action Act world-state agent-state))]  
           [new-agent-state (cdr (_apply-action Act world-state agent-state))]) ;noua stare a agentului obtinuta in urma efectuarii actiunii Act
     (cond
       [(finished? agent-color goal world-state) ]
       [(empty? A) #f]
       [else (forward new-world-state goal  (all-actions new-world-state new-agent-state agent-color) (append Path (list Act)) new-agent-state agent-color)]
    )
 )))

(display "sefwrfwerwr")
(member? 'room1 (_all-rooms test-state))

;;MEGATAG
(define check-move-action
  (λ (args agent-state world-state)
    (display (_all-rooms world-state))
    (display "\n")
    (display (first args))
    (display (second args))
    (display (or (not (member? (first args) (_all-rooms world-state)))
...
;;TAGLOCATION
(define get-location2
  (λ (world-state )
    (cadar (filter (λ (x) (equal? (car x) 'location)) world-state))
    )
  )

(define get-location
  (λ (agent-state )
    (cadr (fourth  agent-state))
    ))
  

;(get-location '((carries red 0) (carries blue 0) (carries gray 0) (location blueWarehouse) (color blue)))

;(get-location test-state)



(define neighbour-rooms 
 (λ (world-state agent-state)
   (map (λ (y) (list (get-location agent-state) (third y))) (filter (λ (x) (and (equal? (first x) 'door) (equal? (second x) (get-location agent-state))) ) world-state))
   )
 )

(define sphere-load-list '( (Load red) (Load blue) (Load gray)))
(define sphere-unload-list '( (Unload red) (Unload blue) (Unload gray)))

sphere-load-list
sphere-unload-list

(display "Vecini: ")
(neighbour-rooms test-state  '((carries red 0) (carries blue 0) (carries gray 0) (location room1) (color red)))

(define move-actions
  (λ (world-state agent-state)
    
     (filter (λ (args) (check-move-action args world-state agent-state )) (neighbour-rooms world-state agent-state))
...
(move-actions  test-state  '((carries red 0) (carries blue 0) (carries gray 0) (location room1) (color red)))

(define load-actions
  (λ (world-state agent-state agent-color)
    '()
    )
  )

(define unload-actions
  (λ (world-state agent-state agent-color)
    '()
    )
  )
...
(define all-actions
  (λ (world-state agent-state agent-color)
    (append (move-actions) (load-actions) (unload-actions))
    )) 


;;la inceput functia forward va primi culoarea agentului ca parametru
;;ca sa nu o calculeze mai tarziu
(define memoryless-agent 
  (λ (goal world-state rest-actions memory)
    (let* ([agent-color (get-agent-color world-state) ])
    (forward world-state goal (all-actions world-state (_initial-agent-state agent-color world-state) ) '() (_initial-agent-state agent-color world-state) agent-color)
    )
  )
  )

(display "True?")
(finished? 'red '(spheres red redWarehouse 4) final-state)
...
;(_in-room-spheres-color 'red 'room1 test-state)
...
(define _apply-action
 ...
 (cdr (_apply-action '(Move redWarehouse room1) test-state (update-agent-state 'red test-state)))
...
(define run
  (λ (scenario red-agent blue-agent (advanced? #f))
    (let* ([initial-world-state (cdr scenario)]
      ...
      (display red-state)  
      (display "\n")
     ;(display (dummy-agent blue-goal  (cdr scenario0) null null) )
...
(run scenario0 dummy-agent dummy-agent)
;;(display (cdr scenario0))
;;(display (_initial-agent-state 'red (cdr scenario0)))
;(memoryless-agent goal (cdr scenario0) null null)
;(display (run scenario0 dummy-agent dummy-agent))
;;(display (dummy-agent  '(spheres red redWarehouse 1) (cdr scenario1) null null))
