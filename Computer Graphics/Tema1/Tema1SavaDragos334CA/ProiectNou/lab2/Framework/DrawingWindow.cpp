...
//int DrawingWindow::width(1320);
//int DrawingWindow::height(720);

int DrawingWindow::width(1024);
int DrawingWindow::height(720);
...
//pentru nava
void DrawingWindow::addObject2D(Spaceship *s)
{
	//objects2D.push_back(o);
	for (int i = 0; i < s->components.size();i++)
		objects2D.push_back(s->components[i]);

}



void DrawingWindow::addObject2D_to_Visual2D(Spaceship *s,Visual2D *v2d)
{
	for (int i = 0; i < s->components.size(); i++)
		v2d->visual2D_objects2D.push_back(s->components[i]);
}


void DrawingWindow::addObject2D_to_Visual2D(Enemy *e, Visual2D *v2d)
{
	for (int i = 0; i < e->components.size(); i++)
		v2d->visual2D_objects2D.push_back(e->components[i]);
}
...
void DrawingWindow::removeEnemy_from_Visual2D(Enemy *e,Visual2D *v){
	for (int i = 0; i < e->components.size(); i++){
		removeObject2D_from_Visual2D(e->components[i],v);
	}
}
