...
public class MedicWindow implements EventHandler<Event> {

    private Stage applicationStage;
    private Scene applicationScene;

    private ArrayList<ReceptionerTableManagement> databaseManagementControllers;
    ...
    public void start() {
        ...
        try {
            ...
            applicationScene = new Scene((Parent) FXMLLoader.load(getClass().getResource(Constants.MEDIC_FXML)));
    ...
    @FXML
    protected void registeredPatientHandler(ActionEvent event) {
    	
    	FXMLLoader loader = new FXMLLoader(getClass().getResource(Constants.REGISTERED_PATIENT_FXML));
        try {
			Parent root = (Parent)loader.load();
            RegisteredPatient controller = loader.<RegisteredPatient>getController();
            controller.setPersonalIdentifier(personalIdentifier);
            controller.init2();
            applicationScene = new Scene(root);
    ...
    @FXML
    protected void programmedPatientHandler(ActionEvent event) {
    	
    	FXMLLoader loader = new FXMLLoader(getClass().getResource(Constants.PROGRAMMED_PATIENT_FXML));
        try {
			Parent root = (Parent)loader.load();
            ProgrammedPatient controller = loader.<ProgrammedPatient>getController();
            controller.setPersonalIdentifier(personalIdentifier);
            controller.init2();
            applicationScene = new Scene(root);
    ...
    @FXML
    protected void writeReportHandler(ActionEvent event) {
    	
    	FXMLLoader loader = new FXMLLoader(getClass().getResource(Constants.REPORT_FXML));
        try {
			Parent root = (Parent)loader.load();
			MedicalReportWindow controller = loader.<MedicalReportWindow>getController();
            controller.setPersonalIdentifier(personalIdentifier);
            controller.init();
            applicationScene = new Scene(root);
    ...
    @FXML
    protected void historyHandler(ActionEvent event) {
    	
    	FXMLLoader loader = new FXMLLoader(getClass().getResource(Constants.HISTORY_FXML));
        try {
			Parent root = (Parent)loader.load();
			History controller = loader.<History>getController();
            controller.setPersonalIdentifier(personalIdentifier);
            controller.init();
            applicationScene = new Scene(root);
                                    ...
                                    if (tab.getText().equals(newTab.getText())) {
                                        try {
                                            ...
                                            if (!databaseManagementControllers.isEmpty()) {
                                            	ReceptionerTableManagement currentTableManagementController = databaseManagementControllers.get(currentIndex);
                                                if (Constants.DEBUG) {
        ...
        if (event.getSource() instanceof MenuItem) {
            Platform.runLater(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!tabExists) {
                        FXMLLoader fxmlLoader = new FXMLLoader(this.getClass().getResource(Constants.MEDIC_TABLE_MANAGEMENT_FXML));
                        ReceptionerTableManagement databaseManagementController = new ReceptionerTableManagement(Utilities.menuEntryToTableName(menuEntry));
